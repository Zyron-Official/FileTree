...
class FileTreeAdapter(
...
) : ListAdapter<FileTreeNode, FileTreeViewHolder>(FileTreeDiffCallback()), FileTreeAdapterUpdateListener {
    ...
    private fun setItemViewLayout(holder: FileTreeViewHolder, node: FileTreeNode) {
        val indentationPx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (14 * node.level).toFloat(), context.resources.displayMetrics).toInt()
    ...
    private fun updateItemViewState(holder: FileTreeViewHolder, node: FileTreeNode, position: Int) {
        ...
        holder.itemView.isSelected = position == selectedItemPosition && node.isExpanded && node.file.isDirectory
    }
    ...
    private fun onBindDirectory(holder: FileTreeViewHolder, node: FileTreeNode) {
        ...
        holder.chevronIconView.setImageDrawable(ContextCompat.getDrawable(context, fileTreeIconProvider.getChevronIcon()))
        holder.chevronIconView.rotation = targetRotation
        holder.chevronIconView.visibility = View.VISIBLE
        holder.fileIconView.setImageDrawable(ContextCompat.getDrawable(context, fileTreeIconProvider.getIconForFolder(node.file)))
        holder.fileNameView.text = node.file.name

        val currentRotation = holder.chevronIconView.rotation
        if (currentRotation != targetRotation) {
            val rotationAnimator = ObjectAnimator.ofFloat(holder.chevronIconView, "rotation", currentRotation, targetRotation).apply {
                duration = 300
